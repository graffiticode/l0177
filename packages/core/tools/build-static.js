// Emits L0177's public static assets into dist/static/. As a child of L0000, L0177 merges
// inherited content from its parent:
//   - lexicon.json: the merged lexicon (base + L0177) — already merged in src/lexicon.ts.
//     (the legacy lexicon.js request path is aliased to it by the API server.)
//   - instructions.md: parent (L0000) instructions concatenated with L0177's.
// The rest (spec.html, language-info.json, scope.json, schema.json, template.gc,
// usage-guide.md) are L0177's own.
...
// 1. lexicon — merged (base + L0177) as plain JSON in lexicon.json. No lexicon.js is written:
...
// 3. instructions.md — parent (L0000) instructions + L0177's. Resolve the parent's
...
// 4. Copy L0177's own verbatim spec assets. spec-directive.md is L0177's
//    per-dialect get_spec directive (consumed by the console's spec-generation
//    service to emit a developer RECIPE instead of a content description).
